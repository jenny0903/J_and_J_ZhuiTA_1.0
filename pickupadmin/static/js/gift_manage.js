...
function display_gift(data){
	...
	var id, name, price, discount, unit, image, icon, position, status ,created_date;
	...
	for(i_gift in data.gift_items){
		...
		if(status == 0){
			$('.management_wrap ul').append(gift_show_tpl.replace('{id}',id).replace('{position}',position).replace('{name}',name).replace('{price}',price).replace('{icon}',icon).replace('{image}',image).replace('{unit}',unit));
		}else{
			$('.management_wrap ul').append(gift_hide_tpl.replace('{id}',id).replace('{position}',position).replace('{name}',name).replace('{price}',price).replace('{icon}',icon).replace('{image}',image).replace('{unit}',unit));
		}
...
$('.manage_btn_new').click(function(){
	if(!$('.manage_btn_new').hasClass('btn_disable')){
		...
		$('.management_wrap ul li.title').append(gift_tpl_add.replace('{id}',"").replace('{position}',"").replace('{name}',"").replace('{price}',"").replace('{icon}',"").replace('{image}',"").replace('{unit}',""));
	}	
});
...
$('.add_gift').die().live('click',function(){
	...
	$.ajax({
		...
		data:'name='+name+'&icon='+icon+'&image='+image+'&price='+price+'&unit='+unit+'&position='+position+'&discount=0&status=1',
		dataType:"JSON",
		async: false,
		success: function(data){
			if( data == '1' ||data==1){
...
$('.update_gift').die().live('click',function(){
	...
	$.ajax({
		...
		data:'id='+giftid+'&name='+name+'&icon='+icon+'&image='+image+'&price='+price+'&unit='+unit+'&position='+position+'&discount=0&status='+status,
		dataType:"JSON",
		async: false,
		success: function(data){
			if( data == '1' ||data==1){
